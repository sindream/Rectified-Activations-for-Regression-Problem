...
if __name__ == "__main__":
    ...
    # Initialize list to collect ground truth depth values
    gt_depth_values = []
    ...
    with torch.no_grad():
        for idx, (input_image, gt_depth) in enumerate(loader_train):
            ...
            # Collect ground truth depth values
            if gt_depth.numel() > 0:
                gt_depth_values.append(gt_depth.cpu().numpy())
    ...
    if num_samples > 0:
        ...
        # Concatenate all ground truth depth values
        all_gt_depth = np.concatenate(gt_depth_values)
        all_disp = 1/all_gt_depth
        # Plot histogram of ground truth depth
        plt.figure(figsize=(8, 6))
        plt.hist(all_disp, bins=100, color='blue', alpha=0.7)
        print(all_disp.mean())
        plt.title('Histogram of Ground Truth Depth')
